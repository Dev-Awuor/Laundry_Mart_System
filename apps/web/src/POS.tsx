...
export default function POS() {
  ...
  function changeQty(id: number, qty: number) {
    setCart((prev) => prev.map((l) => (l.id === id ? { ...l, qty: Math.max(1, qty || 1) } : l)))
  }
        ...
        <div className="grid grid-cols-2 gap-2 md:grid-cols-3 lg:grid-cols-4">
          {services.map((s) => (
            <button
              key={s.id}
              onClick={() => addToCart(s)}
              className="h-16 rounded-lg border bg-white px-3 text-left hover:bg-gray-50 disabled:opacity-50"
              disabled={!s.is_active}
              title={!s.is_active ? "Inactive" : s.name}
            >
              <div className="truncate text-sm font-medium">{s.name}</div>
              <div className="text-xs text-gray-500">KSh {kes.format(Number(s.base_price))}</div>
            </button>
          ))}
          {services.length === 0 && (
      ...
      <div className="space-y-3">
            ...
            <tbody>
              {cart.map((l) => (
                <tr key={l.id} className="border-t">
                  ...
                  <td className="px-3 py-2">KSh {kes.format(l.price)}</td>
                  <td className="px-3 py-2">KSh {kes.format(l.price * l.qty)}</td>
                  <td className="px-3 py-2">
        ...
        <div className="rounded-lg border bg-white p-4 text-sm">
          ...
          <div className="flex items-center justify-between">
            ...
            <span>KSh {kes.format(totals.subtotal)}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-gray-500">Taxable</span>
            <span>KSh {kes.format(totals.taxable)}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-gray-500">VAT 16%</span>
            <span>KSh {kes.format(totals.vat)}</span>
          </div>
